...
class Particle:
    ...
    def set_initial_conditions(self, v0, angle, x0, y0, dt, r, C, A, m):
        ...
        self.brzina.append(np.sqrt((self.vx[-1])**2+(self.vy[-1]**2)))
        self.dt=dt
        self.r=r
        self.C=C
        self.A=A
    ...
    def __move(self):
        ...
        self.ay.append(-9.81-((self.r*self.C*self.A/2*self.m)*(self.vy[-1])**2)*np.sign(self.vy[-1]))
        self.brzina.append(np.sqrt((self.vx[-1])**2+(self.vy[-1]**2))*np.sign(self.vx[-1]))
